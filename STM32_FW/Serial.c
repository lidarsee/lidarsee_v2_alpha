...
void USART1_IRQHandler(void)
{
  char ch;
   ...
   {
     ch = USART_ReceiveData(USART1);       // get received character
     ...
     if ((bufferWritePos != (bufferReadPos-1))&&(bufferWritePos < BUFFER_SIZE))
     {
      	receiveBuffer[bufferWritePos] = ch;
      	bufferWritePos++;
     }
...
void Serial_WriteWords(u16 *word, u16 numWords)
{
	...
	{
		  USART_SendData(USART1, (word[i] & 0x00FF));

		  /* Loop until the end of transmission */
		  while (USART_GetFlagStatus(USART1, USART_FLAG_TC) == RESET) {}

		  USART_SendData(USART1, ((word[i] & 0xFF00) >> 8));
...
void Serial_Print(char *chrBuf)
{
	...
	while (chrBuf[i] != 0)
	{
		  USART_SendData(USART1, chrBuf[i]);
